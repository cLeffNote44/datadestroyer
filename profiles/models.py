...
class UserProfile(models.Model):
    ...
    auto_delete_messages = models.BooleanField(
        default=False, help_text=_("Automatically delete messages after retention period")
    )

    auto_delete_documents = models.BooleanField(
        default=False, help_text=_("Automatically delete documents after retention period")
    )

    # Security settings
    enable_two_factor = models.BooleanField(
        default=False, help_text=_("Enable two-factor authentication")
    )

    enable_encryption = models.BooleanField(
        default=True, help_text=_("Enable end-to-end encryption for sensitive data")
    )

    # Anonymization settings
    anonymize_on_delete = models.BooleanField(
        default=True, help_text=_("Anonymize data instead of hard delete")
    )
    ...
    last_data_export = models.DateTimeField(
        null=True, blank=True, help_text=_("Last time user exported their data")
    )

    last_data_purge = models.DateTimeField(
        null=True, blank=True, help_text=_("Last time user purged their data")
    )
    ...
    def schedule_data_deletion(self):
        ...
        return DeletionRequest.objects.create(
            user=self.user, retention_policy=self.default_retention_policy
        )
...
class SecuritySettings(models.Model):
    ...
    profile = models.OneToOneField(
        UserProfile, on_delete=models.CASCADE, related_name="security_settings"
    )
    ...
    allowed_ips = models.JSONField(
        default=list, blank=True, help_text=_("List of allowed IP addresses")
    )

    blocked_ips = models.JSONField(
        default=list, blank=True, help_text=_("List of blocked IP addresses")
    )

    # Security notifications
    notify_on_login = models.BooleanField(
        default=True, help_text=_("Send notification on successful login")
    )

    notify_on_failed_login = models.BooleanField(
        default=True, help_text=_("Send notification on failed login attempts")
    )

    notify_on_data_export = models.BooleanField(
        default=True, help_text=_("Send notification when data is exported")
    )

    notify_on_data_deletion = models.BooleanField(
        default=True, help_text=_("Send notification when data is deleted")
    )

    # Recovery settings
    recovery_email = models.EmailField(blank=True, help_text=_("Recovery email address"))

    recovery_phone = models.CharField(
        max_length=20, blank=True, help_text=_("Recovery phone number")
    )
