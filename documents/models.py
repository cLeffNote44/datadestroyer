...
class DocumentCategory(models.Model):
    ...
    is_sensitive = models.BooleanField(
        default=False,
        help_text=_('Mark if this category contains sensitive data')
    )
    default_retention_days = models.IntegerField(
        default=90,
        help_text=_('Default retention period in days')
    )
...
class Document(models.Model):
    ...
    encryption_key_id = models.CharField(
        max_length=255,
        blank=True,
        help_text=_('Reference to encryption key')
    )
    ...
    retention_date = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_('Date when document should be deleted')
    )
    deletion_date = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_('Actual deletion date')
    )
